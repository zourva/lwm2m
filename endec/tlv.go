...
const (
	TlvFieldIdentifierType   = 192
	TlvFieldIdentifierLength = 32
	TlvFieldTypeOfLength     = 24
	TlvFieldLengthOfValue    = 7
)

const (
	TypeFieldTypeObjectInstance   = 0   // 00
	TypeFieldTypeResourceInstance = 64  // 01
	TypeFieldTypeMultipleResource = 128 // 10
	TypeFieldTypeResource         = 192 // 11
)
...
//
// 2-0: 3 bit unsigned integer indicating Length of the Value

func CreateTlvTypeField(identType byte, value interface{}, ident uint16) byte {
	var typeField byte

	valueTypeLength, _ := core.GetValueByteLength(value)
